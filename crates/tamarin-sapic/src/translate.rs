...
use crate::annotation::{lower_for_translation, ProcessAnnotation};
...
pub(crate) fn propagate_names<A: GoodAnnotation>(
    ...
    crate::process_walk::walk_mut(&mut p, Vec::<String>::new(), |node, names| {
        ...
        ann.parsed_mut().process_names = names.clone();
...
pub(crate) fn translate(
    ...
    let an_proc_pre: Process<ProcessAnnotation<LVar>, SapicLVar> =
        propagate_names(lower_for_translation::<LVar>(plain));
...
fn is_pos_neg_formula(f: &LNFormula) -> (bool, bool) {
    ...
    tamarin_utils::stack::ensure_sufficient_stack(|| match f {
        ...
        ProtoFormula::Conn(c, l, r) => {
            let l = is_pos_neg_formula(l);
            let r = is_pos_neg_formula(r);
            match c {
                Connective::And | Connective::Or => and2(l, r),
                Connective::Imp => and2(swap(l), r),
                // Preserve the upstream Iff polarity expression and its association.
                Connective::Iff => and2(swap(l), and2(swap(r), l)),
            }
        }
    })
}
...
mod tests {
    ...
    pub(super) fn core_ctx() -> TransCtx {
        TransCtx {
    ...
    #[test]
    fn deep_polarity_fold_uses_small_stack() {
        tamarin_test_support::on_stack(256 * 1024, || {
            let mut f = action_atom("K");
            for _ in 0..8192 {
                // Iff's asymmetry must survive nested scalar folding.
                f = action_atom("A").iff(f);
            }
            assert_eq!(is_pos_neg_formula(&f), (true, false));
        });
    }
