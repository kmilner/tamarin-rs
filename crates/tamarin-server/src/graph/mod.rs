...
pub struct Graph<'a> {
    ...
    pub system: &'a System,
    /// The compressed/simplified copy [`Graph::repr`] was computed from.  Only
    /// the record PORTS an edge endpoint renders as come from these nodes,
    /// matching the `dsConcs`/`dsPrems` state HS fills while emitting the repr's
    /// nodes (Dot.hs:264-268) and reads back in `dotGenEdge` (Dot.hs:403-406).
