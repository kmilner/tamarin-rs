    ...
    H: Clone,
{
    ...
    loop {
        ...
        let mut result = FormulaBox::new(match input {
    ...
    fn deep_prefix_opens_once_on_small_stack() {
        std::thread::Builder::new()
            ...
            .spawn(|| {
                let count = 8192;
                let mut f: LNFormula =
                    ProtoFormula::Atom(ProtoAtom::EqE(bound(count - 1), bound(0)));
                for _ in 0..count {
                    f = ProtoFormula::for_all(hint("x", LSort::Msg), f);
                }
                let (vars, qua, body) =
                    open_formula_prefix(&f, &mut PreciseFreshState::nothing_used());
