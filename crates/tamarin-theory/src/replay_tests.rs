...
use crate::theory::{proof_chain, ProofTree};
...
fn invalid_deep_proof_replay_and_searched_lifecycle_use_bounded_stack() {
    ...
    tamarin_test_support::on_stack(256 * 1024, move || {
        ...
        for (depth, roundtrip) in [(2030, true), (8192, false)] {
            let mut stored = proof_chain(depth, ProofMethod::Simplify, ProofMethod::Sorry(None));
            ...
            assert_eq!(copied.nodes().count(), depth + 2);
            for debug in [format!("{copied:?}"), format!("{copied:#?}")] {
                assert_eq!(debug.matches("ProofNode {").count(), depth + 2);
            }
            assert!(!crate::proof_skeleton::render(&copied).is_empty());
            assert_eq!(
...
fn replay_scopes_restore_after_deep_child_error() {
    ...
    tamarin_test_support::on_stack(256 * 1024, move || {
        ...
        let stored = proof_chain(
            512,
            ProofMethod::SolveGoal(repeated),
