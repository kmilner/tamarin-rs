...
fn file_phase(path: &Path, root: &Path) -> FileReport {
    ...
    // The formulas are read from the parse tree carrying the macro and
    // predicate expansion `elaborate` applies to them, since the elaborated
    // lemma and restriction store the internal formula this test's second
    // route builds.
