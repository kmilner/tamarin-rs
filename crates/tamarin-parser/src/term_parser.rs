...
impl<'a> Parser<'a> {
    pub(super) fn iterative_term(&mut self, eqn: bool, min: usize) -> Result<Term, ParseError> {
        self.walk_term::<false, false>(eqn, min, &mut formula_parser::Groups::default())
    }

    pub(super) fn grouped_term(
        &mut self,
        groups: &mut formula_parser::Groups<'a>,
        diagnostic: bool,
    ) -> Result<Term, ParseError> {
        if diagnostic {
            self.walk_term::<true, true>(false, 0, groups)
        } else {
            self.walk_term::<true, false>(false, 0, groups)
        }
    }

    // Diagnostic recognition uses the identical grammar and delimiter-error
    // unwinding. A successful cached group needs only its end position: its
    // discarded children cannot affect syntax/arity checks in an enclosing term.
    // Retain variable/nullary heads for the relation parser's node-sort checks.
    // Specialization keeps memoization off the ordinary AST-building path.
    fn walk_term<const GROUPED: bool, const DIAGNOSTIC: bool>(
        &mut self,
        eqn: bool,
        min: usize,
        groups: &mut formula_parser::Groups<'a>,
            ...
            while let Some(frame) = next.take().or_else(|| frames.pop()) {
                ...
                match frame {
                    Frame::Expr(min) => {
                        if GROUPED {
                            self.skip_ws();
                            if let Some(outcome) = groups.term(self.save().offset)
                                && (DIAGNOSTIC || outcome.result.is_err())
                            {
                                self.lx = outcome.end.clone();
                                self.sort_suffix_consumed = outcome.explicit_sort;
                                value = Some(outcome.result.clone()?);
                                next = Some(Frame::Infix(min));
                                continue;
                            }
                        }
                        let opening = self.save().offset;
                        match self.term_head(eqn)? {
                            TermHead::Value(v) => {
                                value = Some(v);
                                next = Some(Frame::Infix(min));
                            }
                            TermHead::Group => {
                                ...
                                frames.push(Frame::Group(opening));
                    ...
                    Frame::Group(opening) => {
                        let result = self.require_punct(")");
                        if GROUPED && (DIAGNOSTIC || result.is_err()) {
                            groups.record_term(
                                opening,
                                formula_parser::TermOutcome {
                                    end: self.lx.clone(),
                                    explicit_sort: self.sort_suffix_consumed,
                                    result: result
                                        .as_ref()
                                        .map(|()| match value.as_ref().unwrap() {
                                            term @ Term::Var(_) => term.clone(),
                                            term @ Term::App(_, args) if args.is_empty() => {
                                                term.clone()
                                            }
                                            _ => Term::NumberOne,
                                        })
        ...
        let result = result.map_err(|mut error| {
            for frame in frames.iter().rev() {
                ...
                if GROUPED && let Frame::Group(opening) = frame {
                    groups.record_term(
                        *opening,
                        formula_parser::TermOutcome {
                            end: self.lx.clone(),
