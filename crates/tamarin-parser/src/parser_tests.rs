...
#[test]
fn iterative_formulas_and_proofs_parse_on_a_small_native_stack() {
...
fn deep_formula_substitution_and_renaming_use_bounded_stack() {
    std::thread::Builder::new()
        ...
        .spawn(|| {
            ...
            rename_bound_formula(&mut formula, &variable("y", 0), &variable("y", 1));
            let mut vars = Vec::new();
            collect_formula_vars(&formula, &mut vars);
            assert_eq!(vars, [variable("y", 1)]);
        })
        .unwrap()
        .join()
        .unwrap();
...
#[test]
fn iterative_processes_parse_on_a_small_native_stack() {
    std::thread::Builder::new()
        .stack_size(64 * 1024)
        .spawn(|| {
            let n = 8192;
...
#[test]
fn include_worklist_preserves_state_and_error_sources_on_a_small_stack() {
    ...
    std::thread::Builder::new().stack_size(128 * 1024).spawn(move || {
        ...
        let names: Vec<_> = parsed.items.iter().filter_map(|item| match item {
            TheoryItem::Rule(rule) => Some(rule.name.as_str()),
            _ => None,
        }).collect();
        let expected: Vec<_> = (0..n).map(|i| format!("Before{i}"))
            .chain((0..n).rev().map(|i| format!("After{i}")))
            .chain(std::iter::once("R".to_owned())).collect();
        ...
        assert!(matches!(error.kind(), ParseErrorKind::WrongFunctionArity { .. }));
