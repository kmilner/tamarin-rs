...
impl<'a> Parser<'a> {
    ...
    /// Stop at an include so its file can be expanded before the next item.
    ...
    fn formula(&mut self) -> Result<Formula, ParseError> {
        self.in_context(ParseContext::Formula, Self::formula_body)
    }
    ...
    fn relational_atom(&mut self) -> Result<Formula, ParseError> {
        ...
        self.relational_atom_after(lhs, lhs_explicit_sort)
    }

    fn relational_atom_after(
        &mut self,
        lhs: Term,
        lhs_explicit_sort: bool,
    ...
    fn term_head(&mut self, eqn: bool) -> Result<TermHead, ParseError> {
        self.in_context(ParseContext::Term, |p| p.term_head_inner(eqn))
    }
    ...
    #[inline(never)]
    fn term_head_inner(&mut self, eqn: bool) -> Result<TermHead, ParseError> {
        self.skip_ws();
...
fn apply_let_bindings(
    ...
    for (var, value) in bindings.iter().rev() {
        let replacement_vars = std::cell::OnceCell::new();
        for f in premises
            .iter_mut()
        ...
        for phi in restrictions.iter_mut() {
            subst_let_formula(phi, var, value, &replacement_vars);
        }
    }
}
...
// Membership-only identity sets: type annotations do not participate in rule
// variable identity, and set iteration never determines fresh-name selection.
type RuleVars = tamarin_utils::FastSet<(String, LSort, u64)>;

fn rule_var_key(var: &VarSpec) -> (String, LSort, u64) {
    (var.name.clone(), var.sort, var.idx)
}

fn subst_let_formula(
    phi: &mut Formula,
    key: &VarSpec,
    val: &Term,
    replacement_vars: &std::cell::OnceCell<RuleVars>,
) {
    // Discover whether capture avoidance is needed before allocating scope or
    // freshness data. Ground replacements and unquantified restrictions keep
    // the ordinary substitution path; the RHS is inspected once per binding.
    let mut captures = false;
    phi.visit_mut(|phi| {
        if captures {
            return false;
        }
        if let Formula::Forall(vars, _) | Formula::Exists(vars, _) = phi {
            ...
            captures = vars
                .iter()
                .any(|v| replacement_vars.contains(&rule_var_key(v)));
        }
        !captures
    });
    if !captures {
        phi.visit_mut(|phi| {
            match phi {
                ...
                Formula::Forall(vars, _) | Formula::Exists(vars, _) => {
                    return !vars.iter().any(|v| same_rule_var(v, key));
                }
                _ => {}
            }
            true
    ...
    // One supply for the whole restriction: reserve original names (including
    // nested binders), RHS names and the substitution domain once. Fresh names
    // never collide with an original or another generated name. Their exact
    // indices may differ from subtree-local freshening, but binding is unchanged.
    let replacement_vars = replacement_vars.get().unwrap();
    let mut fresh = FormulaFresh {
        replacement: replacement_vars,
        used: RuleVars::default(),
        next: tamarin_utils::FastMap::default(),
    };
    collect_formula_vars(phi, &mut fresh.used);
    fresh.used.insert(rule_var_key(key));

    // Binding lookup refers to ORIGINAL names. Inserted RHS trees are never
    // visited, so neither substitution nor alpha-renaming can capture them.
    // Explicit scope exits avoid recursion and copying the environment at forks.
    type Identity = (String, LSort, u64);
    enum Work<'a> {
        Visit(&'a mut Formula, bool),
        Restore(Vec<(Identity, Option<u64>)>),
    }
    let mut bound = tamarin_utils::FastMap::default();
    let mut pending = vec![Work::Visit(phi, true)];
    while let Some(work) = pending.pop() {
        let (phi, active) = match work {
            Work::Restore(previous) => {
                for (name, index) in previous.into_iter().rev() {
                    if let Some(index) = index {
                        bound.insert(name, index);
                    } else {
                        bound.remove(&name);
                    }
                }
            ...
            Work::Visit(phi, active) => (phi, active),
        };
        match phi {
            Formula::Atom(atom) => visit_atom_terms_mut(atom, |term| {
                term.visit_mut(|term| {
                    if let Term::Var(v) = term {
                        if let Some(index) = bound.get(&rule_var_key(v)) {
                            v.idx = *index;
                        } else if active && same_rule_var(v, key) {
                            *term = val.clone();
                            return false;
                        }
                    }
            ...
            Formula::Forall(vars, body) | Formula::Exists(vars, body) => {
                let active = active && !vars.iter().any(|v| same_rule_var(v, key));
                let mut renames = Vec::with_capacity(vars.len());
                for var in vars {
                    let original = rule_var_key(var);
                    if active && replacement_vars.contains(&original) {
                        var.idx = fresh.index(var);
                    }
                    renames.push((original, var.idx));
                }
                // Preserve the previous left-to-right renaming behavior for
                // repeated names within a single parser binder group.
                let previous = renames
                    .into_iter()
                    .rev()
                    .map(|(name, index)| {
                        let old = bound.insert(name.clone(), index);
                        (name, old)
                    })
                    .collect();
                pending.push(Work::Restore(previous));
                pending.push(Work::Visit(body, active));
            }
            Formula::Not(body) => pending.push(Work::Visit(body, active)),
            Formula::And(a, b)
            | Formula::Or(a, b)
            | Formula::Implies(a, b)
            | Formula::Iff(a, b) => {
                pending.push(Work::Visit(b, active));
                pending.push(Work::Visit(a, active));
            }
...
struct FormulaFresh<'a> {
    replacement: &'a RuleVars,
    used: RuleVars,
    next: tamarin_utils::FastMap<(String, LSort), u64>,
}

impl FormulaFresh<'_> {
    fn index(&mut self, var: &VarSpec) -> u64 {
        let next = self.next.entry((var.name.clone(), var.sort)).or_default();
        // Every occupied index is examined at most once per name/sort family.
        // Starting at zero also handles an original u64::MAX index safely.
        while self.used.contains(&(var.name.clone(), var.sort, *next))
            || self
                .replacement
                .contains(&(var.name.clone(), var.sort, *next))
        {
            *next += 1;
        }
        self.used.insert((var.name.clone(), var.sort, *next));
        *next
    }
}

fn collect_term_vars(term: &Term, out: &mut RuleVars) {
    term.visit(|term| {
        if let Term::Var(v) = term {
            out.insert(rule_var_key(v));
        }
    });
}

fn collect_formula_vars(formula: &Formula, out: &mut RuleVars) {
    formula.visit(|formula| match formula {
        Formula::Atom(atom) => collect_atom_vars(atom, out),
        Formula::Forall(vars, _) | Formula::Exists(vars, _) => {
            for var in vars {
                out.insert(rule_var_key(var));
            }
        }
        _ => {}
    });
...
fn collect_atom_vars(atom: &Atom, out: &mut RuleVars) {
    match atom {
...
fn subst_let_atom(atom: &mut Atom, key: &VarSpec, val: &Term) {
    visit_atom_terms_mut(atom, |term| subst_let_term(term, key, val));
}

fn visit_atom_terms_mut(atom: &mut Atom, mut visit: impl FnMut(&mut Term)) {
    match atom {
        Atom::Eq(a, b) | Atom::Less(a, b) | Atom::LessMset(a, b) | Atom::Subterm(a, b) => {
            visit(a);
            visit(b);
