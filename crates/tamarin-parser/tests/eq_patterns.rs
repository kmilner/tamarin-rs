...
//! the line/column of each error is the oracle's.  Where the whole frame is
//! byte-identical the test pins it; where only the `expecting` labels differ
//! (the port's no-alternative fallback and post-variable hangovers carry
//! fewer labels — a pre-existing frame class that the planned codespan error
//! reporting replaces wholesale) the test pins the position and the
//! `unexpected` token, and the comment records the oracle's frame.
...
fn embedded_msr_facts_and_restrict_formulas_take_patterns() {
    ...
    let p::Formula::Atom(p::Atom::Eq(lhs, _)) = &restrictions[0] else {
...
fn eq_starts_no_term_outside_pattern_positions() {
    ...
    // no-alternative fallback (whose frame wording differs, same class as
