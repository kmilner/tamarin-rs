...
fn document_spine_walks_use_bounded_stack() {
    tamarin_test_support::on_stack(256 * 1024, || {
        ...
        let transforms: [(&str, fn(Doc) -> Doc, &str); 6] = [
            ("beside", |d| d.beside(Doc::text("!")), "!"),
            ("one_liner", one_liner, ""),
            ("singleton", |d| fsep(vec![d]), ""),
            ("sep", |d| sep(vec![d, Doc::text("!")]), " !"),
            ("empty_fill", |d| fsep(vec![d, Doc::empty()]), ""),
            ("above", |d| d.above(Doc::text("!")), " !"),
        ];
        for (name, transform, suffix) in transforms {
            let transformed = transform(doc.clone());
            assert_eq!(
                transformed.one_line_render(),
                "x".repeat(100_000) + suffix,
                "{name}"
            );
        }
        for _ in 0..100_000 {
...
#[test]
fn narrow_fill_constructs_only_linear_suffix_work() {
    // Unary application wrappers must reuse an already normalized choice
    // spine, not fill its growing prefix again at every enclosing term.
    for (depth, items) in [(128, 2), (1024, 2), (128, 20), (1024, 20)] {
        FILL_STEPS.set(0);
        let mut doc = fsep(vec![Doc::text("a"); items]);
        for _ in 0..depth {
            doc = Doc::text("h(")
                .beside(fsep(vec![doc]))
                .beside(Doc::text(")"));
        ...
        assert!(
            FILL_STEPS.get() < 10,
            "depth={depth} items={items}: {} fill steps",
            FILL_STEPS.get()
        );
        assert_eq!(
            doc.render_with(FLAT_WIDTH, FLAT_WIDTH),
            "h(".repeat(depth) + &vec!["a"; items].join(" ") + &")".repeat(depth)
        );
    }
    ...
    for combine in [fsep, fcat] {
        for count in [1024, 4096] {
            FILL_STEPS.set(0);
            let doc = combine(vec![Doc::text("xxxxxxxx"); count]);
            assert!(
                FILL_STEPS.get() < 10,
                "construction should stop at the first item"
            );
            let output = doc.render_with(69, 69);
            assert_eq!(output.matches("xxxxxxxx").count(), count);
            assert!(FILL_STEPS.get() < count * 100, "work: {}", FILL_STEPS.get());
        }
    }
}

#[test]
fn suspended_construction_and_transforms_drop_on_small_stack() {
    tamarin_test_support::on_stack(256 * 1024, || {
        ...
        let mut doc = Doc::Suspend(LazyRight::new(move || {
            drop(capture);
            panic!("unrequested construction forced");
        }));
        for i in 0..20_000 {
            doc = match i % 4 {
                0 => doc.beside(Doc::text("tail")),
                1 => sep(vec![doc, Doc::text("tail")]),
                2 => fsep(vec![doc, Doc::text("tail")]),
                _ => one_liner(doc.nest(2)),
            };
        }
        drop(doc);
...
#[test]
fn suspended_empty_choices_keep_smart_constructor_semantics() {
    for above in [false, true] {
        let input = Doc::Union(
            rc(Doc::Suspend(LazyRight::new(|| Doc::Empty))),
            rc(Doc::text("unused")),
        );
        let output = if above {
            above_nest(input, false, 0, Doc::Empty)
        } else {
            beside_inner(input, false, Doc::Empty)
        };
        assert_eq!(output.one_line_render(), "");
