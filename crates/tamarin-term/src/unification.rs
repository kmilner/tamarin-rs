    ...
    F: Fn(&C) -> LSort,
{
    ...
    match (&l, &r) {
        ...
        (Term::App(lf, la), Term::App(rf, ra))
            if lf == rf && la.len() == ra.len() && matches!(lf, FunSym::NoEq(_) | FunSym::List) =>
        {
    ...
    F: Fn(&C) -> LSort,
{
    match p {
        ...
        Term::App(pf @ (FunSym::NoEq(_) | FunSym::List), pargs) => match t {
            Term::App(tf, targs) if tf == pf && targs.len() == pargs.len() => {
