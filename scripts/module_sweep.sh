#!/bin/bash
# -m spthy/spthytyped/msr parity sweep: RS vs HS, translate-only (no proving).
# Corpus: parity_corpus.txt + every examples/sapic/**/*.spthy (deduped).
# A file where BOTH sides fail with the same rc + normalized stderr counts as
# agreement. Documented residuals resolve to LEDGERED via
# scripts/sweep_expected.tsv. FAMILY=1 restricts to scripts/module_family.txt.
# Output: TSV rows  <file>\t<module>\t<OK|DIFF|ERROR|LEDGERED>\t<detail>
set -u
. "$(dirname "$0")/sweep_common.sh"
OUT=${OUT:-$REPO/scripts/results/module_sweep.tsv}
mkdir -p "$(dirname "$OUT")"

list_files() {
  if [ "${FAMILY:-0}" = 1 ]; then
    sed 's/#.*//;/^\s*$/d' "$REPO/scripts/module_family.txt" | sed "s|^|$EXAMPLES/|" \
      | while read -r f; do
          if [ -f "$f" ]; then echo "$f"; else echo "WARNING: family entry missing: $f" >&2; fi
        done
    return
  fi
  sed 's/#.*//;/^\s*$/d' "$REPO/scripts/parity_corpus.txt" | sed "s|^|$EXAMPLES/|"
  find "$EXAMPLES/sapic" -name '*.spthy' | sort
}

# Known pre-existing divergence (NOT the -m feature's): RS's derivation-check
# stage emits the "[Open Chains] Too many chain constraints" warning twice
# where HS emits it once (visible at the plain path too). Collapse consecutive
# duplicates of that exact line on both sides.
nerr() { awk '!(/^\[Open Chains\] Too many chain constraints/ && $0 == prev) { print } { prev = $0 }'; }

one() {
  f=$1; m=$2
  d=$(mktemp -d)
  hs_run "$d" "$f" "module-$m-dct30" --derivcheck-timeout=30 -m=$m; hrc=$?
  grun "$RS_BIN" --with-maude="$MAUDE" --derivcheck-timeout=30 -m=$m "$f" > "$d/rs.out" 2> "$d/rs.err"; rrc=$?
  if [ $hrc -ge 124 ] || [ $rrc -ge 124 ]; then echo -e "$f\t$m\tERROR\ttimeout/kill hs=$hrc rs=$rrc" >> "$OUT"
  elif [ $hrc -ne $rrc ]; then echo -e "$f\t$m\tDIFF\trc hs=$hrc rs=$rrc" >> "$OUT"
  elif ! diff -q <(norm < "$d/hs.out") <(norm < "$d/rs.out") >/dev/null; then echo -e "$f\t$m\tDIFF\tstdout" >> "$OUT"
  elif ! diff -q <(norm < "$d/hs.err" | nerr) <(norm < "$d/rs.err" | nerr) >/dev/null; then echo -e "$f\t$m\tDIFF\tstderr" >> "$OUT"
  else echo -e "$f\t$m\tOK\t-" >> "$OUT"; fi
  rm -rf "$d"
}
export -f one grun norm nerr hs_run hs_fingerprint
export HS_BIN RS_BIN MAUDE OUT TIMEOUT HS_CACHE

rs_stale_check
LIST=$(list_files | sort -u)
: > "$OUT"
sweep_banner module_sweep "$(($(echo "$LIST" | wc -l) * 3))"
echo "$LIST" | while read -r f; do for m in spthy spthytyped msr; do echo "$f $m"; done; done \
  | xargs -P "$JOBS" -n 2 bash -c 'one "$0" "$1"'
sweep_finish "$OUT" module 3
